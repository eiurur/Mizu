...
module.exports = class DLSiteDownloader extends Downloader {
  getDownloadOptions() {
    return {};
  }
  ...
  async write(items) {
    ...
    for (const item of items) {
      try {
        await super.wait();
        if (this.debug) console.log(`Now :: ${item.images.large}`);

        // download and write to local folder
        const filename = await imageDownloader.download({
          url: item.images.large,
          filename: `${RandomStringGenerator.createHash(item.images.large)}${
            item.ext
          }`,
