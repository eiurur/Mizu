...
//           // WebMは除外
//           if (/(ＷｅｂＭ|WebM)/i.test(title)) {
//             console.log('WebM => ', title);
//             return;
//           }

//           // MP4は除外
//           if (/(ＭＰ４|MP4)/i.test(title)) {
//             console.log('MP4 => ', title);
//             return;
//           }


//           const size_pattern = /Size:(\d*)x(\d*)/;
//           const size = size_pattern.exec(title);
//           console.log(size);
//           // console.log(title);
...
//           var largeImg = previewImg.replace('c.sankakucomplex.com', 'cs.sankakucomplex.com').replace('/preview', '');
//           // console.log(largeImg)

//           const filename = previewImg.split('/').pop();
//           const sampleFilename = "sample-" + filename;
//           var mediumImg = previewImg.replace('c.sankakucomplex.com', 'cs.sankakucomplex.com').replace('/preview', '/sample').replace(filename, sampleFilename);
//           // console.log(mediumImg)

//           const originalImageWidth = size[1] - 0;
//           const originalImageHeight = size[2] - 0;
//           var url = (originalImageWidth >= 1080 && originalImageHeight >= 1080) ? mediumImg : largeImg;

//           // const ext = path.extname(mediumImg);

//           const ext = (/(ＧＩＦ|GIF)/i.test(title)) ? '.gif' : path.extname(url);
//           if(/(ＧＩＦ|GIF)/i.test(title)) {
//             mediumImg = mediumImg.replace(path.extname(mediumImg), ext);
//             largeImg = largeImg.replace(path.extname(largeImg), ext);
//             url = url.replace(path.extname(url), ext);
//           }
