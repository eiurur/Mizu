  ...
  downloadIllusts() {
    return new Promise((resolve, reject) => {
      ...
      this._illustList.forEach( (illust, illustIdx) => {
        ...
        const filePathAbsolute = path.resolve(this._directory, filename);
        ...
        // ダウンロードタスクの用意
        const opts = {
          url: illust.mediumImg || illust.mediumImg,
          directory: this._directory,
          filename: filename,
        };
        promises.push(Delayer.delayPromise(illustIdx * 3 * 1000).then( _ => this.download(opts) ));
      });
