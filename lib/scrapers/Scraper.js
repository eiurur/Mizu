...
module.exports = class Scraper {
  ...
  async downloadIllusts() {
    ...
    this._illustList = await Promise.all(this._illustList.map((illust, illustIdx) => {
    ...
    const result = await Promise.all(this._illustList.map((illust, idx) => {
      const opts = {
        url: illust.url,
  ...
  rejectIllustsFaildToDownload(downloadedIllustList) {
    return this._illustList.filter(illust => illust && downloadedIllustList.includes(illust.filename));
  }
