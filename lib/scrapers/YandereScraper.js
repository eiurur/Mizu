    ...
    return new Promise((resolve, reject) => {
      ...
      .then( result => {
        ...
        result.$('#post-list-posts li').each( function(idx) {
          ...
          const sourceURLRelative = result.$(this).find('.thumb').attr('href');
          const sourceURL = `${DOMAIN}${sourceURLRelative}`;
          // console.log(sourceURL);

          const largeImgURL = result.$(this).find('.directlink').attr('href');
          ...
          const params = { title, sourceURL, largeImgURL, mediumImgURL, url, ext };
