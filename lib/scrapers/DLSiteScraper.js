...
module.exports = class DLSiteScraper extends Scraper {
  ...
  async scrape({ range, type, category, sub, affiliateId, skip = 0, limit = 20 }) {
    ...
    return affiliateId ? takedContents.map((content) => this.attachAffiateId(content, affiliateId)) : takedContents;
  }
  ...
  attachAffiateId(content, affiliateId) {
    content.source = content.source.replace('work/=/product_id', `dlaf/=/link/work/aid/${affiliateId}/id`);
    return content;
  }
  ...
  extract(result) {
    ...
    result.$('#ranking_table tr').each(function (idx) {
      const workName = result.$(this).find('.work_name a').text();
      const makerName = result.$(this).find('.maker_name a').text();
      const title = `${workName} / ${makerName}`;

      const sourceRelative = result.$(this).find('.work_name a').attr('href');
      const source = result.$(this).find('.work_name a').attr('href');

      const mediumRelative = result.$(this).find('.work_thumb img').attr('src') || result.$(this).find('.work_thumb img').attr('data-src');
      ...
      const large = medium.replace(thumbnailUrl[0], '').replace('/resize/', '/modpub/');
