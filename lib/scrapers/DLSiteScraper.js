...
module.exports = class DLSiteScraper extends Scraper {
  ...
  async scrape({ range, category, workTypes }) {
    const request = this.normalizeRequestOptions({ range, category, workTypes });
  ...
  extract(result) {
    ...
    result.$('#ranking_table tr').each(function (idx) {
      ...
      // 6件目以降の('.work_img_main_popup img')が空なので。やむなく置換処理で対応
  ...
  normalizeRequestOptions({ range, category, workTypes }) {
    const term = ((term) => {
      switch (term) {
        case 'days':
          return 'day';
