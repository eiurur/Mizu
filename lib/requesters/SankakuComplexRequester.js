...
const HOSTNAME = 'chan.sankakucomplex.com';
const DOMAIN = 'https://chan.sankakucomplex.com';
const ENDPOINT = `${DOMAIN}/ja/post/`;
const POST_SHOW_URL = `${DOMAIN}/ja/post/show/`;

module.exports = class SankakuComplexRequester {
  constructor(params) {
    this.term = params.term;
    this.date = moment().format('YYYY-MM-DD');
    this.directory =
      params.directory || `sankaku_complex_${this.term}_${moment(this.date).format('YYYY_MM_DD')}`;
    this.page = 1;
    this.requestURL = '';
    this.requestOpts = {
      headers: {
    ...
    return new Promise((resolve, reject) => {
      ...
      const opts = {
        ...
        query: {
          ...
          month: moment(this.date).month() + 1, // Accepts numbers from 0 to 11.(Quote by moment.js)
  ...
  _addIllustList(illustList) {
    return new Promise((resolve, reject) => {
      ...
      illustList.forEach((illust) => {
        ...
        const sourceURL = POST_SHOW_URL + illust.id;
        const largeImgURL = `https:${this._normalizeImageURL(illust.file_url)}`;
        const mediumImgURL = `https:${this._normalizeImageURL(illust.sample_url)}`;
